...
public class CityJsonObjectParser implements CityJsonParser {
    ...
    @Override
    public void parseCities(InputStream in,
        ...
        for (int i = 0; i < citiesArray.length(); i++) {
            // Check whether we are canceled and stop
            if (cancelHandle != null && cancelHandle.isCanceled()) {
                Log.w(LOG_TAG, "parseCities: canceled");
                break;
            }
